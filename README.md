...
当前 README 对应版本：`v1.5.1（2026.4.16）`
...
- 新增静态资源缺失专门异常处理：`NoResourceFoundException` 返回 404，并按 warn 级别记录日志，减少 ERROR 噪音。
- 新增可复用清理任务入口：`repairFileCleanupTask.cleanBrokenTaskResourceUrls()`，可在 Quartz 中直接执行失效 URL 清理。

#### v1.5.1 任务配置建议（可选）

- 在后台“定时任务”中新增任务，调用目标填写：`repairFileCleanupTask.cleanBrokenTaskResourceUrls()`。
